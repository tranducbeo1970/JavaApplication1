...
p('Tôi có trách nhiệm quản lý, điều hành Phòng Nghiên cứu phát triển (NCPT); kiểm tra tiến độ, chất lượng và hồ sơ nhiệm vụ khoa học và công nghệ (KH&CN) do Phòng chủ trì; tham gia lập và tổ chức thực hiện kế hoạch được giao.')
...
p('Một là, rà soát hồ sơ và các tồn tại có liên quan; xác định người thực hiện, thời hạn hoàn thành và kiểm tra kết quả khắc phục.')
p('Hai là, kiểm soát hồ sơ, tiến độ theo từng mốc; kịp thời xử lý vướng mắc, báo cáo nội dung vượt thẩm quyền, không để tồn tại kéo dài.')
p('Ba là, đánh giá kỹ tính khả thi và nguồn lực khi đề xuất kế hoạch; phối hợp chặt chẽ với các đơn vị liên quan để hoàn thiện công việc, hồ sơ.')
...
out=Path(__file__).parent.parent/'Ban_kiem_diem_Tran_Duc_3_trang.docx';d.save(out);print(out)
