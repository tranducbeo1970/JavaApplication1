 ...
 *
 */
public class ReceiveX400MailNew {

    /*
    // Mandatory P7 configuration settings. You must change these values for this program to work with P7
    //private static String p7_message_store_presentation_address = "\"3001\"/URI+0000+URL+itot://10.10.1.1:3001";
    private static String p7_message_store_presentation_address = "\"3001\"/URI+0000+URL+itot://10.64.1.5:3001";
    //private static String p7_user_or_address = "/OU=VVTSMHSA/O=AFTN/PRMD=VV/ADMD=ICAO/C=XX/";
   ...
   // private static String p7_user_or_address = "/CN=VVTSSWIM/OU=VVTS/O=VVTS/PRMD=VIETNAM/ADMD=ICAO/C=XX/";
...
//private static String p7_user_or_address = "/OU=VVTSMHSA/O=AFTN/PRMD=VV/ADMD=ICAO/C=XX/";
    ...
    private static String p3_channel_presentation_address = "\"593\"/URI+0000+URL+itot://10.10.1.1";
    private static String p3_user_or_address = "/OU=VVTSPPPP/O=AFTN/PRMD=VIETNAM/ADMD=ICAO/C=XX/";
...
//                else {
        ...
        if (status == X400_att.X400_E_NOERROR) {
            ...
            if (rm.GetType() == X400_att.X400_MSG_MESSAGE) {
                ...
                System.out.println("The Subject is '" + rm.getSubject() + "'");
                 System.out.println("The Date rm.getMessageDeliveryTime is '" + rm.getMessageDeliveryTime().toString() + "'");
                 System.out.println("The Date is '" + rm.getMessageDeliveryTimeAsDate().toString() + "'");
                 System.out.println("The getMessageSubmissionTime is '" + rm.getMessageSubmissionTime().toString() + "'");
    ...
    public static void listTheMailbox(P7BindSession connection) throws X400APIException {
        ...
        ArrayList<ListResult> list_array = connection.listMailbox("200108041516Z", entry_class, only_new_messages);
    ...
    static public void listMessages() throws X400APIException {
        ...
        SimpleDateFormat DAY_CURRENT_FORMAT = new SimpleDateFormat("yyMMdd010000'Z'");

        String since = DAY_BEGIN_FORMAT.format(new Date());
        String until = DAY_CURRENT_FORMAT.format(new Date());
        System.out.println("since " + since + " to " + until);

        int code = X400ms.x400_ms_listexauxpribefore(connection, since, until, entryClass, -1, X400_att.X400_PRIORITY_ANY, messageList);
        // ex(session, since, entryClass, messageList);
        ...
        for (i = 1;; i++) {
            ...
            code = X400ms.x400_ms_listgetintparam(messageList, X400_att.X400_N_MS_SEQUENCE_NUMBER, i);
            ...
            X400ms.x400_ms_listgetstrparam(messageList, X400_att.X400_S_MESSAGE_SUBMISSION_TIME, i, s);

            int status = X400ms.x400_ms_listgetstrparam(messageList, X400_att.X400_S_CONTENT_STRING, i, txt);

            if ((status != X400_att.X400_E_NOERROR) && (status != X400_att.X400_E_NOSPACE)) {
                System.out.println("Error");
            }
            X400ms.x400_ms_listgetstrparam(messageList, X400_att.X400_S_MESSAGE_IDENTIFIER, i, mid);
            X400ms.x400_ms_listgetstrparam(messageList, X400_att.X400_S_SUBJECT, i, sub);

            //int  con = X400ms.x400_ms_checkconnection(connection);
            //MSMessage rm = new MSMessage();
            //X400ms.x400_ms_msgget(connection, i, rm);
            //Message fwdMsg = new Message();
            //BodyPart bp =new BodyPart() ; 
            //X400ms.x400_ms_msggetbodypart(rm, i,bp);
            System.out.println("Message : " + i + " " + s + " " + code + " " + " " + mid + " [" + sub + "] " + txt);

        }

        //List<Index> indexes = parse(messageList, entryClass);
        X400ms.x400_ms_listfree(messageList);
//
//        return indexes;
    }

//    static private List<Index> parse(MSListResult msListResult, int entryClass) {
//
...
//
//    }
    static public void Index(MSListResult msListResult, int index) {
