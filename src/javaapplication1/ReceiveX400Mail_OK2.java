 ...
 *
 */
public class ReceiveX400Mail_OK2 {
    ...
    private static final String p7_user_or_address = "/CN=VVVVYMYX/OU=VVVV/O=VVTS/PRMD=VIETNAM/ADMD=ICAO/C=XX/";
...
//< DIA CHI P3
    ...
    //private static String p3_channel_presentation_address = "\"593\"/URI+0000+URL+itot://10.64.1.107";
...
public static void showMessage(ReceiveMsg rm) throws X400APIException {
    int status = 0;
    String logFilePath = "G:\\P3_VVTSATMR.log";

    // mở log append (không ghi đè)
    try (PrintWriter logWriter = new PrintWriter(new FileWriter(logFilePath, true))) {

        // hàm tiện ích: vừa in ra console vừa ghi log
        java.util.function.Consumer<String> log = (msg) -> {
            System.out.println(msg);
            logWriter.println(msg);
            logWriter.flush(); // đảm bảo ghi ngay
        };

        if (status == X400_att.X400_E_NOERROR) {
            int ext = rm.getIntParam(AMHS_att.ATS_N_EXTENDED);
            if (isExtend) {
                log.accept("+++++++++++++++++++ The message is EXTENDED FORMAT +++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
                ...
                String ohi = getStrParam(rm, AMHS_att.ATS_S_OPTIONAL_HEADING_INFO);

                log.accept("AtsFilingTime=" + AtsFilingTime);
                log.accept("setAtsPriority=" + setAtsPriority);

                if (ohi != null && !ohi.isEmpty()) {
                    log.accept("OHI = [" + ohi + "]");
                } else {
                    log.accept("OHI = <empty>");
                }

                log.accept("________________________________________________\n");
            } else {
                        log.accept("------------------- The message is BASIC FORMAT ------------------------------------------------------------");
            }

            log.accept("The type is '" + rm.getTypeAsString() + "'");

            if (rm.GetType() == X400_att.X400_MSG_MESSAGE) {
                log.accept("The message is from '" + rm.getFrom() + "'");
                log.accept("The message primary recipients are '" + rm.getToRecipients() + "'");
                if (rm.getCcRecipients() != null)
                    log.accept("The message copy recipients are '" + rm.getCcRecipients() + "'");
                if (rm.getBccRecipients() != null)
                    log.accept("The message blind copy recipients are '" + rm.getBccRecipients() + "'");

                log.accept("The Subject is '" + rm.getSubject() + "'");
                log.accept("The Date is '" + rm.getMessageSubmissionTimeAsDate().toString() + "'");
                log.accept("The Time is '" + rm.getMessageSubmissionTime() + "'");
                log.accept("The Priority is '" + rm.getX400Priority().toString() + "'");

                JRSecurityLabel securityLabel = rm.getJRSecurityLabel();
                if (securityLabel != null) {
                    log.accept("The Security Label is '" + securityLabel.getXML() + "'");
                }

                int numOfBodyparts = rm.getNumberOfBodyparts();
                switch (numOfBodyparts) {
                    case 0: log.accept("There are no bodyparts"); break;
                    case 1: log.accept("There is one bodypart"); break;
                    default: log.accept("There are " + numOfBodyparts + " bodyparts"); break;
                }

                for (int i = 1; i <= numOfBodyparts; i++) {
                    Bodypart bp = rm.getBodypart(i);
                    log.accept("Bodypart number " + i + " is of type " + bp.getTypeAsString());

                    if (bp instanceof BodypartIA5Text) {
                        BodypartIA5Text bpt = (BodypartIA5Text) bp;
                        log.accept(bpt.getTextContent());
                        ...
                        log.accept("Bodypart size 0 = " + gtbp.getSize() + "\n" + content + "\n+++++++++\n" + result + "\n###########################");
                        if (save_bodyparts_in_files)
                            gtbp.saveBP("D:\\gt.bp");
                    } else if (bp instanceof BodypartFTBP) {
                        BodypartFTBP ftbp = (BodypartFTBP) bp;
                        log.accept(ftbp.getStringRepresentation());
                        if (ftbp.getCreationDate() != null)
                            log.accept("The creation date of the file is " + ftbp.getCreationDate());
                        if (save_bodyparts_in_files)
                            ftbp.saveFTBPInDir("bodyparts");
                    } else if (bp.getType() == Bodypart_Type.BODYPART_BINARY) {
                        log.accept("Bodypart size 1 = " + bp.getSize());
                        if (save_bodyparts_in_files)
                            bp.saveBP("d:\\duc.bin");
                        ...
                        log.accept(fwd.getStringRepresentation());
                    }
                }
            } else if (rm.GetType() == X400_att.X400_MSG_REPORT) {
                log.accept("The DR content is:\n" + rm.getReportContentAsText());
            }

            rm.finishWithMessage(0, 0);
        } else {
            log.accept("Problems reading the message: status = " + status);
        }
    ...
    public static void showMessageOLD(ReceiveMsg rm) throws X400APIException {
        int status = 0;
