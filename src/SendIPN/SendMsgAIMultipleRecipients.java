...
/**
 * Submits an ATS message with explicit, separate transfer-envelope and P22
 * primary-recipient lists.
 */
public final class SendMsgAIMultipleRecipients {
    ...
    private static final String P7_BIND_USER_OR_ADDRESS
            = "/CN=VVBMYMYX/OU=VVBM/O=VVTS/PRMD=VIETNAM/ADMD=ICAO/C=XX/";

    private static final String P7_USER_PASSWORD = "amhs";

    /* Originator written into the submitted message. */
    ...
    private static final String[] PRIMARY_RECIPIENT_OR_ADDRESSES = {
        ...
        "/CN=VDNZQZA/OU=VVDN/O=VVDN/PRMD=VIETNAM/ADMD=ICAO/C=XX/",
        "/CN=VVTSXXXX/OU=VVTS/O=VVNB/PRMD=VIETNAM/ADMD=ICAO/C=XX/"
    };

    private static final String[] ENVELOPE_RECIPIENT_OR_ADDRESSES = {
        "/CN=VVBMZTZX/OU=VVBM/O=VVTS/PRMD=VIETNAM/ADMD=ICAO/C=XX/",
        "/CN=VVBMYDYX/OU=VVBM/O=VVTS/PRMD=VIETNAM/ADMD=ICAO/C=XX/",
        "/CN=VVTSYMYX/OU=VVTS/O=VVTS/PRMD=VIETNAM/ADMD=ICAO/C=XX/",
        "/CN=VVDNYMYX/OU=VVDN/O=VVTS/PRMD=VIETNAM/ADMD=ICAO/C=XX/",
    ...
    private static final String MESSAGE_TEXT
            = "TEST MESSAGE WITH MULTIPLE EXPLICIT ENVELOPE AND PRIMARY RECIPIENTS\r\n"
            + "FROM: VVBMYMYX\r\n"
            + "PRIMARY RECIPIENT COUNT: 12\r\n"
            + "ENVELOPE RECIPIENT COUNT: 12";

    private SendMsgAIMultipleRecipients() {
    }
    ...
    private static void buildAtsMessage(X400Msg message)
        ...
        message.setStringparam(
                ...
                "SENDMSGAI MULTIPLE EXPLICIT P1/P22 RECIPIENT TEST"
        );
        message.setIntParam(X400_att.X400_N_CONTENT_TYPE, 22);
        message.setStringparam(AMHS_att.ATS_S_TEXT, MESSAGE_TEXT);
    }

    private static void addEnvelopeRecipients(X400Msg message)
            throws X400APIException {
        for (String address : ENVELOPE_RECIPIENT_OR_ADDRESSES) {
    ...
    private static void printConfiguration() {
        ...
        printAddresses("Envelope", ENVELOPE_RECIPIENT_OR_ADDRESSES);
